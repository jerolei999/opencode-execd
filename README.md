...
1. an OpenCode plugin that overrides only the built-in `bash` tool;
2. a normal OCI service image that runs the HTTP worker and OpenSandbox `execd` together.
...
Publish the package to the company npm registry or bake this repository into the OpenCode image. Then configure OpenCode:
...
The endpoint can also be supplied through `OPENCODE_EXECD_WORKER_URLS`. Multiple comma-separated URLs enable client-side failover and deterministic session affinity:

```bash
export OPENCODE_EXECD_WORKER_URLS=http://worker-1:9010,http://worker-2:9010
export OPENCODE_EXECD_WORKER_TOKEN=worker-secret
```
...
- Registers a custom tool named `bash`, which replaces OpenCode's built-in tool without modifying OpenCode Core.
- Keeps the native `command`, `timeout`, and `workdir` arguments.
- Calls OpenCode's `bash` permission check before dispatch.
- Sends cancellation when the user aborts a tool call.
- Best-effort calls `/release` when a session is deleted.
- Leaves all filesystem tools local because both sides see the same CubeFS mount.
